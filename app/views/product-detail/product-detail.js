...
export default class ProductDetailPage extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isConfirmTextViewDisabled: false,
            localize: {
                "views": "Views",
                "today": "Today",
                "unit": "Unit",
    ...
    onClickOrangeMoneyBtn(){
        this.setState({isConfirmTextViewDisabled: true});
    }
                ...
                <View style={{flex:this.state.flexBottom, alignSelf:'stretch'}}>
                    <ScrollView  contentContainerStyle={{flex:1,alignItems:'center',justifyContent:'flex-end',alignSelf:'stretch'}}>
                        ...
                        {!this.state.is_notification &&
                            ...
                            onPress={()=>this.onClickOrangeMoneyBtn()}>
                        ...
                        <View style={{height:40,backgroundColor:'#eee',flexDirection:'row',marginLeft:15,marginRight:15,marginBottom:20,borderRadius:5}}>
                            <View style={{justifyContent:'center',alignItems:'flex-start',height:40,flex:0.90}}>
                                <TextInput
                                    ...
                                    editable={this.state.isConfirmTextViewDisabled}
                                />
                            </View>
    ...
    onPressSaveFromSubscribeModal(){
        if(!this.state.is_notification) {
            ...
            if(confirmationCode != '987654') {
                setTimeout(()=>{
                    Alert.alert(
                        'App',
                        'Payment unsuccessful, Confirmation code is not valid',
                        [
                            { text: 'OK', onPress: () => console.log('OK Pressed') }
                        ]
                    )
        ...
        this.setState({ animating: true, change_subscription:true, isConfirmTextViewDisabled: false });
    ...
    renderChart(){
        ...
        let styleChart = {height: 120, flexDirection: 'column'};
        if(Platform.OS == 'android'){
            styleChart = {height: 180};
        }
        if(this.state.chartData[0].length > 2){
            return (
                <View style={{ marginTop: 30, height:160 }}>
                    ...
                    <ScrollView horizontal={true} contentContainerStyle={styleChart}>
                        <View>
